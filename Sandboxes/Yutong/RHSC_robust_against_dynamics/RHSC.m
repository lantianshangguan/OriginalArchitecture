function RHSC_out = RHSC(ya, va, psia, dpsia, steeringangle_act, oldu, yd, psid)
...
%     r_thre = 0.071;
    ...
    A11 = [1,T_s,T_s*vx,0,0;
        0,(C_f1+C_r1)/m/vx*T_s+1,0,((C_f1*lf-C_r1*lr)/m/vx-vx)*T_s,-C_f1*T_s/m;
        0,0,1,T_s,0;
        0,(C_f1*lf-C_r1*lr)/Iz/vx*T_s,0,(C_f1*lf^2+C_r1*lr^2)/Iz/vx*T_s+1,-C_f1*lf*T_s/Iz;
        0,0,0,0,1-T_s/fa];
...
%         alfa_f{k} = (xvar{k}(2)+lf*xvar{k}(4))/vx-uvar{k};
        ...
        alfa_f{k} = (xvar{k}(2)+lf*xvar{k}(4))/vx-xvar{k}(5);
...
%         constraints = [constraints,-0.15 <= uvar{k+1} -uvar{k} <= 0.15];
        
%         constraints = [constraints,-.6*T_s <= uvar{1} - pastu <= .6*T_s];
    ...
    RHSC_out = controller{{[ya;va;psia;dpsia;steeringangle_act],yd*ones(N+1,1),psid*ones(N+1,1),oldu}};
else
    RHSC_out = controller{{[ya;va;psia;dpsia;steeringangle_act],yd*ones(N+1,1),psid*ones(N+1,1),oldu}};
end
end